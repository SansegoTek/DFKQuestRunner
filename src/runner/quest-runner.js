...
async function checkForQuests() {
  try {
    ...
    let runningQuests = activeQuests.filter(
      (quest) => !doneQuests.includes(quest)
    );
...
async function evaluateQuests(activeQuests) {
  ...
  for (const quest of config.quests) {
    if (
      quest.professionHeroes.length > 0 &&
      !questingHeroes.includes(quest.professionHeroes[0])
    ) {
      var staminaGood = await evaluateQuestHeroesStamina(
        quest,
    ...
    if (
      quest.nonProfessionHeroes.length > 0 &&
...
async function evaluateQuestHeroesStamina(quest, maxAttempts, professional) {
  ...
  console.log(
    `${professional ? 'Professional' : 'Non-professional'} ${
      quest.name
    } quest is not ready to start. Lowest stamina is ${lowestStamina}`
  );
  ...
  try {
    ...
    let receipt = await tryTransaction(
      () =>
        questContract
          .connect(wallet)
          .startQuest(quest.heroes, quest.address, quest.attempts),
      2
    );
...
async function completeQuest(heroId) {
  try {
    ...
    let receipt = await tryTransaction(
      () => questContract.connect(wallet).completeQuest(heroId),
      2
    );
